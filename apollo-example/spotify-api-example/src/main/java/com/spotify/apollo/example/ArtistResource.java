...
public class ArtistResource {
    ...
    return client
        ...
        .thenComposeAsync(
            response -> {
              ...
              Request topTracksRequest =
                  Request.forUri(
                      String.format(
                          "%s/%s/top-tracks?country=%s", ARTIST_API, topArtistId, country));
              return client.send(topTracksRequest);
            })
        .thenApplyAsync(
            response -> Response.ok().withPayload(parseTopTracks(response.payload().get().utf8())))
        // This .exceptionally doesn't provide any additional value,
        // but it shows how you could handle exceptions
        .exceptionally(
            throwable ->
                Response.forStatus(
                    Status.INTERNAL_SERVER_ERROR.withReasonPhrase("Something failed")));
  }

  /**
   * Parses an artist top tracks response from the <a
   * href="https://developer.spotify.com/web-api/get-artists-top-tracks/">Spotify API</a>
   *
  ...
  /**
   * Parses the first artist id from a JSON response from a <a
   * href="https://developer.spotify.com/web-api/search-item/">Spotify API search query</a>.
   *
   * @param json The json response
