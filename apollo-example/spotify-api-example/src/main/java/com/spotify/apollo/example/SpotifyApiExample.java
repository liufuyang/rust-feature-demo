 ...
 *
 * <p>It uses asynchronous routes that calls the Spotify API to fetch some data and then process it.
 */
final class SpotifyApiExample {
  ...
  static void init(Environment environment) {
    ...
    DummyResource dummyResource = new DummyResource(objectMapper);

    environment
        .routingEngine()
        .registerRoutes(albumResource.routes())
        .registerRoutes(artistResource.routes())
        .registerRoutes(dummyResource.routes())
