...
public class AlbumResource {
  ...
  CompletionStage<Response<ArrayList<Album>>> getAlbums(RequestContext requestContext, String tag) {
    ...
    return client
        ...
        .thenApplyAsync(
            response -> Response.ok().withPayload(parseAlbumData(response.payload().get().utf8())));
  }

  /**
   * Parses an album response from a <a
   * href="https://developer.spotify.com/web-api/album-endpoints/">Spotify API album query</a>.
   *
  ...
  /**
   * Parses the album ids from a JSON response from a <a
